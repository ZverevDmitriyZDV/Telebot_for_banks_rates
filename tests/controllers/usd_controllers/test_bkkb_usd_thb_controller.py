...
from tests.fixtures_for_bkkb_client import get_last_data_response, get_family_responce, get_lak_update_rates, \
    get_usd_update_rates, make_responce_object
...
def test_get_usd_to_thb_rate(get_last_data_response,get_usd_update_rates):
    conf = BKKBConfiguration(token="TOKEN")
    usd_to_thb = LastUSDToTHBRates(conf=conf)
    fake_responce_update = make_responce_object(get_last_data_response, 200)
    fake_responce_get_x = make_responce_object(get_usd_update_rates,200)
    usd_to_thb.client.format_update_data = MagicMock(return_value=fake_responce_update)
    usd_to_thb.client.get_x_rate = MagicMock(return_value = fake_responce_get_x)
